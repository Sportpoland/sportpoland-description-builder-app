...
const AllegroDescriptionEditor = () => {
  ...
  const exportTemplates = () => {
    ...
    link.href = url;
    link.download = 'szablony-allegroeditor.json';
    link.click();
      ...
      reader.onload = (e) => {
        try {
          ...
          if (Array.isArray(importedTemplates)) {
            // Nadpisz ID aby uniknąć konfliktów
            const processedTemplates = importedTemplates.map(template => ({
              ...template,
              id: Date.now() + Math.random(),
              date: new Date().toLocaleString('pl-PL') + ' (importowany)'
            }));
            
            setTemplates(prev => [...prev, ...processedTemplates]);
            alert(`Zaimportowano ${processedTemplates.length} szablonów!`);
          } else {
            alert('Nieprawidłowy format pliku szablonów!');
          }
        } catch (error) {
  ...
  const duplicateTemplate = (template) => {
    ...
    if (newName && newName.trim()) {
      ...
      alert(`Kopia szablonu "${newName}" została utworzona!`);
    }
  };
